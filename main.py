...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        default_factory=lambda: ["choice", "iop", "format"],
...
def iou_timestamp_reward(
    ...
    for content, sol in zip(completions, timestamps):  # Added video_durations
...
def iou_timestamp_reward_v2(
    ...
    for content, sol, duration in zip(
        completions, timestamps, durations
...
def iop_timestamp_reward(
    completions, timestamps, **kwargs
):
    rewards = []
    current_time = datetime.now().strftime("%d-%H-%M-%S-%f")

    for content, sol in zip(completions, timestamps):
        reward = 0.0
        parsed_times = parse_timestamp_output(content)
        start_time, end_time = 0, 0
        ...
        if parsed_times:
            start_time, end_time = parsed_times
            pred_len = max(1e-6, end_time - start_time)

            intersection = max(0, min(end_time, e) - max(start_time, s))
            reward = intersection / pred_len
        ...
        if os.getenv("DEBUG_MODE") == "true":
            with open(os.getenv("LOG_PATH"), "a", encoding="utf-8") as f:
                f.write(f"[IoP]\n")
                f.write(f"pred: {start_time} - {end_time}\n")
                f.write(f"gt  : {gt_start} - {gt_end}\n")
...
def choice_reward(
    completions, answer, question, choices, **kwargs
):
    """
    completions: 模型输出
    answer: GT 正确选项，如 'A'
    从 <choice>...</choice> 中取最后一个 A/B/C/D
    """
    rewards = []
    current_time = datetime.now().strftime("%d-%H-%M-%S-%f")
    ...
    choice_block_pattern = re.compile(
        r"<choice>(.*?)</choice>", re.DOTALL | re.IGNORECASE
    )
    abcd_pattern = re.compile(r"[ABCD]", re.IGNORECASE)

    for content, gt_choice in zip(completions, answer):
        reward = 0.0
        pred_choice = None

        choice_blocks = choice_block_pattern.findall(content)
        if choice_blocks:
            last_block = choice_blocks[-1]
            matches = abcd_pattern.findall(last_block)
            if matches:
                pred_choice = matches[-1].upper()
                if pred_choice == gt_choice.upper():
                    reward = 1.0
        ...
        if os.getenv("DEBUG_MODE") == "true":
            log_path = os.getenv("LOG_PATH")
            with open(log_path, "a", encoding="utf-8") as f:
                f.write("[Choice Reward]\n")
                f.write(f"Question    : {question}\n")
                f.write(f"Choices     : {choices}\n")
                f.write(f"GT choice   : {gt_choice}\n")
                f.write(f"Pred choice : {pred_choice}\n")
                f.write(f"Reward      : {reward}\n")
...
def format_reward(completions, **kwargs):
    ...
    pattern = re.compile(
        r"<think>.*?</think>\s*"
        r"<answer>\s*\d+\.?\d*\s*(to|and)\s*\d+\.?\d*\s*</answer>\s*"
        r"<choice>\s*[ABCD]\s*</choice>",
