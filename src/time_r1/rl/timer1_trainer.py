...
QUESTION_TEMPLATE_GQA_v1 = """You are given a video, a question, and four candidate answers.
...
2. Identify the video moments that support the correct answer using the exact timestamps.
...
Instructions:
- First, output your reasoning process within <think> </think> tags.
- Your reasoning should include references to relevant video moments.
- When mentioning a video moment, use <timestep>start time to end time</timestep> in seconds, precise to two decimal places.
- Do NOT include the answer or final timestamps inside the <think> section.

Then, output:
- the start and end times of the moment that best supports your answer, within <answer> </answer>, formatted as "start time to end time".
- the selected answer choice (A, B, C, or D) within <choice> </choice>.
...
class TimeR1_Trainer(Trainer):
    ...
    def make_conversation_video(self, example):
        if self.prompt_type == "gqa_v1":
            prompt_text = QUESTION_TEMPLATE_GQA_v1.format(
    ...
    def compute_loss(
        ...
        if self.accelerator.is_main_process:
            for idx, (example, completion) in enumerate(zip(inputs, completions)):
                print("\n" + "=" * 60)
                print(f"[SAMPLE {idx}]")
