...
# ================== timestamp overlay switches ==================
ENABLE_TIMESTAMP_OVERLAY = 1
DEBUG_SAVE_TIMESTAMP = 0
DEBUG_DIR = "debug_video_frames"
# ================================================================
...
def overlay_timestamp_on_frame(
    frame: torch.Tensor,
    timestamp_sec: float,
    debug_id: str | None = None,
) -> torch.Tensor:
    """
    frame: (C, H, W), RGB, float tensor [0,255] or [0,1]
    """
    import cv2
    import numpy as np
    from sklearn.cluster import KMeans
    ...
    max_label = np.argmax(np.bincount(labels.flatten()))
    mask = (labels == max_label).astype(np.uint8)

    num_labels, labels_im = cv2.connectedComponents(mask)

    best_label, max_area = 0, 0
    ...
    background_mask = (labels_im == best_label)
    ...
    for name, (cx, cy) in corners.items():
        d = (xs - cx) ** 2 + (ys - cy) ** 2
        idx = np.argmin(d)
        corner_points[name] = (xs[idx], ys[idx])

    # ---------- 3. 选最靠角的点 ----------
    closest_point = min(
        corner_points.items(),
        key=lambda kv: (kv[1][0] - corners[kv[0]][0]) ** 2
        + (kv[1][1] - corners[kv[0]][1]) ** 2,
    )[1]
    ...
    bg_color = img[py, px][::-1]  # RGB->BGR
    bg_brightness = brightness(bg_color)

    candidates = [(0, 0, 0), (255, 255, 255),(255,0,0), (0,255,0),(0,0,255)]
    text_color = max(
        candidates, key=lambda c: abs(brightness(c) - bg_brightness)
    )
    ...
    x = min(max(0, px - tw // 2), w - tw)
    y = min(max(th, py + th // 2), h - baseline)
    ...
    # ---------- debug 保存 ----------
    if DEBUG_SAVE_TIMESTAMP and debug_id is not None:
        os.makedirs(DEBUG_DIR, exist_ok=True)
        cv2.imwrite(
            os.path.join(DEBUG_DIR, f"{debug_id}.jpg"),
...
def fetch_video(
    ...
    if isinstance(ele["video"], str):
        ...
        if ENABLE_TIMESTAMP_OVERLAY:
            ...
            # 当前帧对应真实时间
                timestamp = video_start + i / sample_fps

                video[i] = overlay_timestamp_on_frame(
                    video[i],
                    timestamp_sec=timestamp,
                    debug_id=f"{os.path.basename(ele['video'])}_f{i}",
                )
